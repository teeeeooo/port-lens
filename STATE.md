...
9. **D0.4 cold-start race control:** raise the polling/result gate on non-button compact-bar `pointerdown` instead of after the 4 px threshold. This targets the startup-refresh result race between pointerdown and first movement while leaving the 4 px movement threshold, grab-ratio IPC, raw position-only `SetWindowPos`, hover lifecycle, and idle polling unchanged. Windows runtime validation is pending.
10. Preserve the validated size-drift fix, hover-window architecture, Open deadlock fix, taskbar behavior, mixed-DPI/multi-monitor handling, startup gating, and PR #3 runtime lifecycle protections. Integrate nothing into PR #4 until D0.4 is evaluated.
