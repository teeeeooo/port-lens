...
6. Direct comparison shows Port Lens and Token Lens use the same Tauri/Tao/WRY versions, but Port Lens D0/historical `cca33ce` used raw Windows `SetWindowPos` while current Token Lens uses Tauri `window.set_position()`. **D0.1** is the next single-variable control: keep all D0 input/cursor/hover logic unchanged and replace only the Windows drag-time movement primitive with `set_position()`.
7. Preserve the validated size-drift fix, hover-window architecture, Open deadlock fix, taskbar behavior, mixed-DPI/multi-monitor handling, startup gating, and PR #3 runtime lifecycle protections. Integrate nothing into PR #4 until a drag candidate passes Windows manual validation.
