...
8. **D0.3 production-shape control:** restore normal 3 s monitored/runtime and 10 s inventory polling while idle; suspend polling/result application only after real drag begins (>4 px); on pointerup/cancel/lost capture, finish the final move, release the drag gate, then perform one catch-up refresh. Drag-active state stays in refs so lifecycle transitions do not render by themselves. Windows runtime validation is pending.
9. Preserve the validated size-drift fix, hover-window architecture, Open deadlock fix, taskbar behavior, mixed-DPI/multi-monitor handling, startup gating, and PR #3 runtime lifecycle protections. Integrate nothing into PR #4 until D0.3 passes Windows manual validation.
