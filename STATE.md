...
Compact dragging now uses a single immediate `start_compact_drag` transaction from mouse-down. The command hides the hover window once, suspends the taskbar topmost keeper, and hands movement to Tauri/Windows native `start_dragging()` without any 4 px threshold, pointer capture, pointermove IPC, or drag-region frontend command. On Windows, compact `Moved`/`Resized` events are intentionally ignored during native movement; a lightweight mouse-button release watcher persists/clamps the final position once and then restores the z-order keeper.
