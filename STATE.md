...
7. Same-stack comparison leaves compact renderer/background workload as a meaningful remaining difference. **D0.2** restores the better raw position-only `SetWindowPos` path and, only for this diagnostic control, suspends the 3 s monitored/runtime polling and 10 s inventory polling while compact and ignores late in-flight refresh results. Local frontend build/fmt/clippy/Rust 36/36/diff-check gates pass; Windows Bundle `35040462376` for `b7f936f231e50cb7995b0f992c8a1e762971a94a` is SUCCESS; Windows runtime validation is pending.
8. Preserve the validated size-drift fix, hover-window architecture, Open deadlock fix, taskbar behavior, mixed-DPI/multi-monitor handling, startup gating, and PR #3 runtime lifecycle protections. Integrate nothing into PR #4 until a drag candidate passes Windows manual validation.
