...
5. D0 is implemented: DOM pointer capture + 4 px threshold, grab-ratio-only move invoke, backend current-cursor resampling, current-monitor/DPI target calculation, and existing position-only movement. First real movement can hide the separate hover HWND in the same backend transaction. No Tauri/Tao native caption drag, per-move UI state update, settings write, z-order correction, size change, or hover tracking is used. Local frontend build, fmt, clippy `-D warnings`, Rust 36/36, and diff-check gates pass. Windows runtime validation remains pending.
6. Preserve the validated size-drift fix, hover-window architecture, Open deadlock fix, taskbar behavior, mixed-DPI/multi-monitor handling, startup gating, and PR #3 runtime lifecycle protections. Integrate nothing into PR #4 until D0 passes Windows manual validation.
