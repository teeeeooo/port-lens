...
Status: **D0 FAILED / D0.1 FAILED / D0.2 IMPLEMENTED, WINDOWS MANUAL VALIDATION PENDING**.
...
Audit result: no architectural blocker was found for D0, so D0 was selected before D1. D0 Windows validation on `f474b0d` / Bundle `34946164626` removed the persistent cursor offset and kept hover-hide/Open correct, but jump/catch-up remained. D0.1 then changed only raw Windows `SetWindowPos` to Tauri `window.set_position()`; Windows manual validation made jump/catch-up worse, so D0.1 also failed. D0.2 restores raw position-only `SetWindowPos` and isolates a remaining Port-Lens-specific variable by suspending compact-mode 3 s/10 s polling and ignoring late in-flight refresh UI updates. If D0.2 still jumps, proceed to D1 rather than further tuning the JS→IPC manual-position family.
