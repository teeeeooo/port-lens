...
Status: **D0 FAILED PRIMARY JUMP GATE / D0.1 IMPLEMENTED, WINDOWS MANUAL VALIDATION PENDING**.
...
Audit result: no architectural blocker was found for D0, so D0 was selected before D1. D0 Windows validation on `f474b0d` / Bundle `34946164626` removed the persistent cursor offset and kept hover-hide/Open correct, but jump/catch-up remained, so D0 failed the primary gate. A same-stack comparison then identified one untested movement difference: Port Lens D0/historical `cca33ce` use raw Windows `SetWindowPos`, while current Token Lens uses Tauri `window.set_position()`. D0.1 changes only that final Windows movement primitive; all pointer capture, threshold, grab-ratio, backend cursor-resampling, and hover behavior stays unchanged. D1 remains fallback if D0.1 also fails.
