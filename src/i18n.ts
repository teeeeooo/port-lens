...
const EN = {
  ...
  previouslyManagedNote: "Recognized as an App previously started by Port Lens, but ownership could not be verified for lifecycle control.",
  reattachedManagedNote: "Verified and reattached to the surviving Port Lens runtime. Stop / Restart are available again.",
  earlyExitNotice: "Start process exited shortly after launch: code {code} after {seconds}s.",
  lastExitNotice: "Last Port Lens start process exited: code {code} after {seconds}s.",
...
const KO: Record<MessageKey, string> = {
  ...
  previouslyManagedNote: "이전에 Port Lens가 시작한 App으로 인식했지만 lifecycle 제어 권한까지는 검증되지 않았습니다.",
  reattachedManagedNote: "기존 Port Lens runtime을 검증해 다시 연결했습니다. Stop / Restart를 다시 사용할 수 있습니다.",
  earlyExitNotice: "Start 직후 root process가 종료되었습니다: {seconds}초 후 종료, code {code}.",
  lastExitNotice: "마지막 Port Lens start process가 종료되었습니다: {seconds}초 후 종료, code {code}.",
